...
// ── Mutable state stored in refs (avoids stale closures in WS handler) ──
interface IBotRefs {
    botState: TBotState;
...
const FreeBots = () => {
    ...
    // ── Render loop: push refs → React state ~300ms ──
    useEffect(() => {
        const tick = () => {
            setRenderTick(n => n + 1);
    ...
    // ── WS message handler (stable, uses refs only) ──
    ...
    // ── Inline helpers called from handler (avoid stale closure by accessing s = b.current) ──
    ...
    // Update tick boxes / signal from refs
        ...
        <div className='free-bots'>
            <div className='free-bots__container'>
                <header className='free-bots__header'>
                    ...
                    <div className='free-bots__header-right'>
                        <div className={'free-bots__badge free-bots__badge--carry' + (s.martLevel >= CFG_STEPS && bState === 'scanning' ? ' active' : '')}>
                            <div className='free-bots__blink' /> CARRYING
                        </div>
                        <div className={'free-bots__badge free-bots__badge--defer' + (s.waitingForDeferred ? ' active' : '')}>
                            <div className='free-bots__blink' /> DEFERRED
                        </div>
                        <div className={'free-bots__badge free-bots__badge--armed' + (s.isArmed ? ' active' : '')}>
                            <div className='free-bots__blink' /> ARMED
                        </div>
                        <div className={'free-bots__badge free-bots__badge--trade' + (bState === 'trading' ? ' active' : '')}>
                            <div className='free-bots__blink' /> TRADING
                        </div>
                        <div className={'free-bots__badge free-bots__badge--scan' + (bState === 'scanning' ? ' active' : '')}>
                            <div className='free-bots__blink' /> SCANNING
                        </div>
                ...
                <div className='free-bots__latency-bar'>
                    <div className='free-bots__lat-item'>
                        <div className='free-bots__lat-label'>Tick Latency</div>
                    ...
                    <div className='free-bots__lat-item'>
                        <div className='free-bots__lat-label'>Order RTT</div>
                    ...
                    <div className='free-bots__lat-sep'>|</div>
                    <div className='free-bots__lat-item'>
                        <div className='free-bots__lat-label'>Entry Proposal</div>
                        <div className={'free-bots__lat-val' + (s.isArmed ? ' good' : s.entryPending ? ' ok' : '')}>
                            {s.isArmed ? 'READY ✓' : s.entryPending ? 'LOADING…' : 'IDLE'}
                        </div>
                    </div>
                    <div className='free-bots__lat-sep'>|</div>
                    <div className='free-bots__lat-item'>
                        <div className='free-bots__lat-label'>Retry Proposal</div>
                        <div className={'free-bots__lat-val' + (s.retryProposalReady ? ' good' : s.retryPending ? ' ok' : '')}>
                            {s.retryProposalReady ? 'READY ✓' : s.retryPending ? 'LOADING…' : 'IDLE'}
                        </div>
                    </div>
                    <div className='free-bots__lat-sep'>|</div>
                    <div className='free-bots__lat-item'>
                        <div className='free-bots__lat-label'>Next Tick ETA</div>
                    ...
                    <div className='free-bots__armed-indicator'>
                        <div className={'free-bots__armed-dot' + (s.isArmed ? ' on' : '')} />
                        <span>{s.isArmed ? 'ARMED ⚡' : 'NOT ARMED'}</span>
                        &nbsp;
                        <div className={'free-bots__retry-dot' + (s.retryProposalReady ? ' on' : '')} />
                        <span>{s.retryProposalReady ? 'RETRY READY ⚡' : 'NO RETRY'}</span>
                    </div>
                </div>

                <div className='free-bots__rotation-bar'>
                    <div className='free-bots__rot-label'>Rotation<br />Assets</div>
                    ...
                    <div className='free-bots__trade-counter'>
                        Trades on asset: <span>{s.tradesOnAsset}</span>
                        &nbsp;|&nbsp; Mart level: <span>{s.martLevel}</span> / <span>{CFG_STEPS}</span>
                    </div>
                </div>
                ...
                <div className='free-bots__pattern-card'>
                    <div className='free-bots__pat-label'>Pattern<br />Tracker</div>
                ...
                <div className='free-bots__grid4'>
                    ...
                    <div className='free-bots__card'>
                        <div className='free-bots__card-label'>📈 Session P&L</div>
                    ...
                    <div className='free-bots__card'>
                        <div className='free-bots__card-label'>🔢 Total Trades</div>
                        <div className='free-bots__card-value free-bots__card-value--blue'>{s.totalTrades}</div>
                        <div className='free-bots__card-sub'>{s.patternsFound} patterns fired</div>
                    </div>
                </div>

                <div className='free-bots__grid3'>
                    <div className='free-bots__card'>
                        <div className='free-bots__card-label'>⚡ Current Stake</div>
                        <div className='free-bots__card-value free-bots__card-value--orange'>${s.currentStake.toFixed(2)}</div>
                        <div className='free-bots__card-sub'>Mart level <span>{s.martLevel}</span> &nbsp;|&nbsp; Step <span>{s.entryStepCount}</span>/<span>{CFG_STEPS}</span></div>
                    ...
                    <div className='free-bots__card'>
                        ...
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'baseline', marginTop: '2px' }}>
                            <div className='free-bots__card-value free-bots__card-value--green' style={{ fontSize: '20px' }}>
                                +${s.bestTrade.toFixed(2)}
                            </div>
                            <div style={{ color: 'var(--fb-dim)', fontSize: '14px' }}>/</div>
                            <div className='free-bots__card-value free-bots__card-value--red' style={{ fontSize: '20px' }}>
                                ${s.worstTrade.toFixed(2)}
                            </div>
                        </div>
                    </div>
                    <div className='free-bots__card'>
                        <div className='free-bots__card-label'>🔄 Current Asset</div>
                        <div className='free-bots__card-value free-bots__card-value--accent'>{ASSET_NAMES[s.currentAsset]}</div>
                        <div className='free-bots__card-sub'>{isConnected ? 'Active' : 'Waiting to connect'}</div>
                    </div>
                </div>

                <div className='free-bots__controls'>
                    <div className='free-bots__controls-title'>⚙ Controls</div>
                    <div className='free-bots__controls-info'>
                        Stake: <span>${CFG_STAKE.toFixed(2)}</span> &nbsp;|&nbsp;
                        Duration: <span>1 Tick</span> &nbsp;|&nbsp;
                        Martingale: <span>×{CFG_MART}</span> &nbsp;|&nbsp;
                        Max Steps: <span>{CFG_STEPS}</span> &nbsp;|&nbsp;
                        Assets: <span>V25 → V100</span>
                    </div>
                    <div className='free-bots__btn-row'>
                        <button className='free-bots__btn free-bots__btn--start' onClick={startBot}
                            disabled={bState !== 'idle' || !isConnected}
                            style={{ display: bState === 'idle' ? 'inline-block' : 'none' }}>
                            ▶ Start Bot
                        </button>
                ...
                <div className='free-bots__log-card'>
                    <div className='free-bots__log-header'>
                        <div className='free-bots__log-title'>📋 Trade History</div>
                        <div className='free-bots__log-count'>{tradeLog.length} trade{tradeLog.length !== 1 ? 's' : ''}</div>
                    </div>
                    ...
                    <div className='free-bots__log-body'>
                        {tradeLog.length === 0 && rotateLog.length === 0 ? (
                            <div className='free-bots__log-empty'>Waiting — connect and start the bot to begin scanning…</div>
                        ) : (
                            <>
                                {rotateLog.map((r, i) => (
